...
do
	COBBLER_LISTEN=$(ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i $PATH_TO_ENV/master-key root@${subnet[1]}.2 dockerctl shell cobbler netstat -lntp | wc -l 2> /dev/null)
	sleep 15
done
